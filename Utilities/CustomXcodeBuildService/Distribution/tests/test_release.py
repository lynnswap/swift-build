...
class BuildTests(unittest.TestCase):
    def setUp(self):
        ...
        self.revision = release.output(
            ["git", "-C", str(self.repository), "rev-parse", "HEAD"]
        )
        (distribution / "ServiceDependencies.resolved").write_text(
            "uncommitted dependency pins\n"
        )
    ...
    def run_command(self, command, **kwargs):
        ...
        self.commands.append(
            (command, environment.copy() if environment is not None else None)
        )
        if command[:2] == ["/usr/bin/xcode-select", "--print-path"]:
            return subprocess.CompletedProcess(
                command, 0, "/Applications/Selected Xcode.app/Contents/Developer\n"
            )
        if command[:3] == ["/usr/bin/xcrun", "xcodebuild", "-version"]:
            return subprocess.CompletedProcess(
                command, 0, "Xcode 27.0\nBuild version 27A5252f\n"
            )
        ...
        if command[0] == sys.executable and command[2] == "stage":
            self.assertEqual(
                Path(command[1]).read_text(), "committed staging implementation\n"
            )
    ...
    def build(
        self, developer_dir="/Applications/Explicit Xcode.app/Contents/Developer"
    ):
        environment = {
            "DEVELOPER_DIR": developer_dir,
            "SWIFTCI_USE_LOCAL_DEPS": "1",
            "SWIFTBUILD_LLBUILD_FWK": "/local/llbuild.framework",
            "SWIFTBUILD_STATIC_LINK": "1",
            "XCBBUILDSERVICE_PATH": "/installed/service",
            "SWBBUILDSERVICE_PATH": "/other/service",
        }
        with patch.object(release, "REPOSITORY_ROOT", self.repository), patch.object(
            release.platform, "system", return_value="Darwin"
        ), patch.object(release.platform, "machine", return_value="arm64"), patch.dict(
            os.environ, environment
        ), patch.object(
            release.subprocess, "run", side_effect=self.run_command
        ):
            release.build(self.arguments)

    def test_build_isolates_committed_source_pins_toolchain_and_service_environment(
        self,
    ):
        self.build()
        ...
        self.assertEqual(
            (self.arguments.output_dir / "source-revision.txt").read_text(),
            self.revision + "\n",
        )
        build_commands = [
            (command, environment)
        ...
        for command, environment in build_commands:
            self.assertEqual(
                ...
                "/Applications/Explicit Xcode.app/Contents/Developer",
            )
            for key in (
            ...
            if "--package-path" in command:
                self.assertTrue(
                    Path(command[command.index("--package-path") + 1]).is_relative_to(
                        source
                    )
                )
                self.assertEqual(command[command.index("--jobs") + 1], "2")
                self.assertEqual(
                    "--force-resolved-versions" in command,
                    command[command.index("--package-path") + 1] == str(source),
                )
        self.assertFalse(
            any(command[0] == "/usr/bin/xcode-select" for command, _ in self.commands)
        )

    def test_service_build_failure_stops_cli_and_staging_after_selecting_toolchain_once(
        self,
    ):
        ...
        self.assertEqual(
            sum(command[0] == "/usr/bin/xcode-select" for command, _ in self.commands),
            1,
        )
        self.assertFalse(
        ...
        for command, environment in self.commands:
            if command[0] == "/usr/bin/xcrun":
                self.assertEqual(
                    environment["DEVELOPER_DIR"],
                    "/Applications/Selected Xcode.app/Contents/Developer",
                )
        self.assertEqual(
            (self.repository / "Package.resolved").read_text(),
            "developer dependency pins\n",
    ...
    def test_existing_output_is_preserved_before_source_extraction(self):
        ...
        self.assertFalse(
            any(
                "archive" in command or "swift" in command
...
class DistributionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.manifest = dict(
            ...
            resourceBundles=list(release.BUNDLES),
    ...
    def package(self, name="release"):
        ...
        release.package(
            argparse.Namespace(build_dir=self.build, output_dir=destination)
        )
        return destination
    ...
    def test_round_trip_preserves_resources_binaries_licenses_and_manifest(self):
        ...
        self.assertEqual(
            (directory / "SHA256SUMS.txt").read_text(), release.checksum_file(directory)
        )
    ...
    def test_repackaging_is_stable_and_refuses_existing_output(self):
        ...
        self.assertEqual(
            (first / release.ARCHIVE).read_bytes(),
    ...
    def test_archive_traversal_links_and_duplicates_are_rejected_before_extraction(
        self,
    ):
        cases = (
            ("../outside", tarfile.REGTYPE),
            ("/absolute", tarfile.REGTYPE),
            ("bin/link", tarfile.SYMTYPE),
            ("bin/hardlink", tarfile.LNKTYPE),
            ("bin/fifo", tarfile.FIFOTYPE),
            ("bin/file", tarfile.REGTYPE),
        )
